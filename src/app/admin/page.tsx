...
import { requireAdmin, ForbiddenError } from "@/lib/server/policy";
...
export default async function AdminPage() {
  try { await requireAdmin(); } catch (error) { if (error instanceof ForbiddenError) notFound(); throw error; }
  const [reports, evidence, disputes, transactions, users, listings] = await Promise.all([
    ...
    prisma.dispute.findMany({ include: { evidence: { where: { uploadStatus: "uploaded" } } }, where: { status: { not: "resolved" } }, orderBy: { filedAt: "asc" }, take: 50 }),
    prisma.transaction.findMany({ orderBy: { createdAt: "desc" }, take: 50, select: { id: true, status: true, type: true, createdAt: true, buyer: { select: { name: true } }, seller: { select: { name: true } } } }),
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-2">
        ...
        <Queue title="Disputes" empty="No open disputes.">{disputes.map((item) => <Row key={item.id} title={item.reason} detail={item.statement + (item.providerStatus ? ` Stripe bank dispute: ${item.providerStatus}.` : "")}>{item.evidence.map(file => <Link key={file.id} className="mt-2 block text-sm underline" href={`/api/disputes/evidence/${file.id}`} target="_blank">{file.originalName}</Link>)}<ModerationControls targetType="dispute" targetId={item.id} actions={["under_review", "resolved"]} /></Row>)}</Queue>
        <Queue title="Recent transactions" empty="No transactions.">{transactions.map((item) => <Row key={item.id} title={`${item.type.replaceAll("_", " ")} - ${item.status.replaceAll("_", " ")}`} detail={`${item.id} · ${item.buyer.name} → ${item.seller.name} · ${item.createdAt.toISOString()}`}>{!["refunded", "cancelled", "capture_pending", "awaiting_payment"].includes(item.status) && <ModerationControls targetType="transaction" targetId={item.id} actions={["refund"]} />}</Row>)}</Queue>
